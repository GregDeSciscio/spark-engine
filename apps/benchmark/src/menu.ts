...
.menu .card { display: flex; flex-direction: column; gap: 8px; text-align: left; cursor: pointer;
  font: inherit; color: inherit; padding: 0 0 18px; border: 1px solid #1c2233; border-radius: 8px;
...
.menu .card.featured { grid-column: 1 / -1; background: linear-gradient(160deg, #131a2e, #0a0d15 70%);
  border-color: #2c3757; }
@media (min-width: 760px) {
  .menu .card.featured { display: grid; grid-template-columns: minmax(0, 420px) minmax(0, 1fr);
    grid-template-rows: auto auto auto; align-content: center; column-gap: 20px; padding: 0 20px 0 0; }
  .menu .card.featured > :not(.thumb) { margin-left: 0; margin-right: 0; }
...
export function renderMenu(container: HTMLElement): () => void {
  ...
  for (const entry of SCENE_ENTRIES) {
    ...
    thumb.src = `thumbs/${entry.definition.name}.jpg`;
    thumb.alt = '';
    thumb.loading = 'lazy';
