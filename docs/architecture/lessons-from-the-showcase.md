...
- A percentage threshold on a one-millisecond measurement is a coin flip. Two consecutive perf runs on a busy machine flagged five different scenes between them with no engine change at all: a ±0.2 ms wobble on a 1 ms GPU time reads as 20%. The harness now takes the best of two passes (contention can only make a number worse), requires an absolute delta as well as a percentage, and says "read this as the machine, not the code" when three quarters of the suite regresses at once. A suite that cries wolf gets ignored, and then it is worth nothing when it is right.
- The agent is part of the machine it is measuring. The run that flagged everything was contended by a browser tab this session had left rendering the mission for forty minutes — the measurement was of the harness competing with itself. Close the preview before running perf, and treat "everything got slower together" as a question about the room, not the code.
- Gameplay needs its own regression layer. Unit tests cover logic with no renderer, the visual suite covers pixels with no behaviour, and between them sat every mechanic that only exists once physics, navigation, animation and AI run together. `pnpm probe` boots the showcase headless on a fixed clock and seed and drives it through `window.__spark.game`. The rule that keeps it useful: assert budgets ("alerted within twenty seconds"), never frame counts, so retuning a constant does not fail the suite — only a broken mechanic does.
- The insert has to be quiet, and that is a testable invariant. Garrisoning the street to eight put a patrol route within 14 m of the spawn: the operator was inside a sight cone before the player had moved, so the mission opened already detected. A patrol route is level data and nothing typechecks it — the probe that stands still at the insert for twenty-five seconds and asserts nobody investigates is what caught it, one hour after the change shipped.
- A probe can only see what the game exposes. Every time a probe wanted a fact — the sector's tier, the objective index, the checkpoint — the fix was to widen the scene's `qa` object, not to reach into the scene from the tool. The probe surface is the game's test seam, and it is worth keeping deliberate.
